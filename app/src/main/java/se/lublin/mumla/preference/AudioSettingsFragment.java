...
import se.lublin.mumla.voicechanger.VoiceChangerPrefs;
...
public class AudioSettingsFragment extends MumlaPreferenceFragment {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        // Anon fork: voice-changer mode picker. Persists to its own
        // SharedPreferences (not the default one Mumla uses) so it's namespaced
        // and easy to migrate. Apply takes effect on the next audio frame
        // captured — no need to reconnect.
        ListPreference voiceChangerPref = findPreference("voice_changer_mode");
        if (voiceChangerPref != null) {
            VoiceChanger.Mode current = VoiceChangerPrefs.load(requireContext());
            voiceChangerPref.setValue(current.name());
            voiceChangerPref.setOnPreferenceChangeListener((pref, newValue) -> {
                VoiceChanger.Mode m;
                try {
                    m = VoiceChanger.Mode.valueOf(String.valueOf(newValue));
                } catch (IllegalArgumentException e) {
                    m = VoiceChanger.Mode.OFF;
                }
                VoiceChangerPrefs.save(requireContext(), m);
                VoiceChanger.shared().setMode(m);
                return true;
            });
        }
    }
