...
public class AudioInput implements Runnable {
    ...
    public AudioInput(AudioInputListener listener, int audioSource, int targetSampleRate,
        ...
        // Anon fork: keep the voice-changer DSP synced with the actual capture rate.
        VoiceChanger.shared().setSampleRate(sampleRate);
    }
    ...
    public void run() {
        ...
        // Anon fork: clear leftover DSP state so audio from a prior session
        // can't bleed into this one. Cheap if mode == OFF.
        VoiceChanger.shared().reset();
        ...
        while(mRecording) {
            ...
            if(shortsRead > 0) {
                // Anon fork: forensic-grade DSP applied in place before the
                // listener (which feeds the Opus encoder).
                VoiceChanger.shared().processInPlace(mAudioBuffer, shortsRead);
